...
def get_lora_config(rank: int = 16, alpha: int = 16) -> LoraConfig:
    return LoraConfig(
        ...
        lora_alpha=alpha,
        target_modules=[
            # Temporal attention only (attn in temporal_transformer_blocks)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for frames, cond_frames in pbar:
            ...
            with torch.no_grad():
                ...
                # Add noise to latents (EDM scheduler, no standard add_noise)
                noise = torch.randn_like(latents)
                # Timesteps as floats in [0, 1] -> EDM sigma
                ...
                # UNet timesteps (normalized log-sigma, as SVD expects)
            ...
            # Encode conditioning frame (first frame)
            with torch.no_grad():
                cond_latents = vae.encode(
                    cond_frames
                ).latent_dist.sample() * vae.config.scaling_factor
                # (B, 4, h, w): no temporal expansion, SVD handles it internally
            ...
            # Concat along channels -> (B, 8, T, h, w)
            unet_input = torch.cat([noisy_latents, cond_latents_expanded], dim=1)

            # SVD expects (B, T, 8, h, w), it does flatten(0,1) internally
...
def run_inference(lora_dir: str | None, image_path: str, output_path: str = "output.mp4"):
    """
    Animates a single landscape image with SVD and the fine-tuned LoRA.
    With lora_dir=None, the pretrained SVD model is used as is (baseline).
    """
    from diffusers import StableVideoDiffusionPipeline
    from peft import PeftModel
    import imageio

    # fp16 on GPU; fall back to fp32 on CPU (much slower, but works)
    ...
    # Load LoRA weights (skipped for the baseline)
    if lora_dir is not None:
        pipe.unet = PeftModel.from_pretrained(pipe.unet, lora_dir)
    pipe.to(device)
    pipe.vae = pipe.vae.to(dtype=dtype)  # SVD VAE has no fp16 variant; force cast after loading
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lora_dir",          type=str,   default=None,   help="LoRA weights (omit for the pretrained SVD baseline)")
    parser.add_argument("--image_path",        type=str,   default=None)
    parser.add_argument("--out",               type=str,   default="output.mp4", help="Output video for --infer")

    args = parser.parse_args()

    if args.infer:
        assert args.image_path, "Provide --image_path for inference"
        run_inference(args.lora_dir, args.image_path, args.out)
    else:
        train(args)
